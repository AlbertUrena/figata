...
- Item view now includes **editorial/detail slides** support:
  - Auto-detects slides inside the item folder (same directory as `source/card/modal`) using image patterns (`<editorial-slug>-slide-<n>.webp`, compact `<editorial-slug>-slide<n>.webp`) and video patterns (`<editorial-slug>-video-slide-<n>.(webm|mp4)`, compact `<editorial-slug>-video-slide<n>.(webm|mp4)`) from known media paths (item-id and source-basename variants, underscore/hyphen, are accepted)
